import { ModelRunErrorCollection, ErrorResponse } from './output'
...
export class FlatGGITError {
    private static DEFAULT_ERROR_DETAILS='An unexpected error has occurred';
    ...
    public static fromErrorResponse(errorResponse: ErrorResponse): FlatGGITError[] {
        return [
    ...
    private static flattenGGITErrorCollections(
        errorClassification: GGITErrorClassification,
        errorCollections?:
        | ModelRunErrorCollection
        | ModelRunErrorCollection[]
    ): FlatGGITError[] {
        return [errorCollections ?? []]
            .flat()
            .flatMap((modelRunCollection) => {
                return [modelRunCollection.Error]
                    .flat()
                    .map((error) => {
                        const errorDetails = error?.['@message'] 
                            ?? modelRunCollection['#text']
                            ?? FlatGGITError.DEFAULT_ERROR_DETAILS;
